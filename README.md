...
獅尾鋸齒黑體基於[思源黑體](https://github.com/adobe-fonts/source-han-sans)的轉角控空和拔腳改造，更加簡明現代化的字體。支援简体中文、繁體中文、韓文與日文；可以免費商用，歡迎大家自由應用、自由優化、自由改作！

獅尾霓黑體的特色是，上半邊圓體+下半邊黑體，一個綜合的字體，「口」字的上沿有一點點像彩虹形狀。
...
```
@font-face {
  font-family: SweiAliasSansCJKtc-Regular;
...
* 演算黑科技將字體的轉角控空改造，請參考 /python/ 目錄下的腳本檔案。透過調整程式碼，也許也可以產生出新的有趣字型。目前的程式應該還有很多錯誤或需要再加強的地方。請先把要處理的字型，透過 FontForge 開啟，並另存 FontForge 的專案為資料夾格式(.sfdir)，最後就可以透過Max的 Python 程式去處理產生出來的檔案。
